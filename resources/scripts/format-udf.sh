...
[[ -b "$DEVICE_PATH" ]] || (
  echo "[-] $DEVICE_PATH either doesn't exist or is not block special" >&2
  false
)
...
[[ -b "$KDEVICE_PATH" ]] || (
  echo "[-] $KDEVICE_PATH either doesn't exist or is not block special" >&2
  false
)
...
if [[ $TOOL_UDF = "$TOOL_MKUDFFS" ]]; then
  ...
  ($SUDO mkudffs --utf8 --blocksize="$FILE_SYSTEM_BLOCK_SIZE" --media-type=hd --udfrev=0x0201 --lvid="$LABEL" --vid="$LABEL" "$KDEVICE_PATH") || (
  ...
  ($SUDO newfs_udf -b "$FILE_SYSTEM_BLOCK_SIZE" -m blk -t ow -r 2.01 -v "$LABEL" --enc utf8 "$KDEVICE_PATH") || (
    echo "[-] Format failed!" >&2
    false
  )
else

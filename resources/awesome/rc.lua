...
globalkeys = gears.table.join(
  ...
  end, { description = "custom border ocr translate screenshot", group = "launcher" }),

  awful.key({ "Mod1" }, "Print", function()
    awful.util.spawn("flameshot_wrapper_ocr")
  end, { description = "custom border ocr clipboard screenshot", group = "launcher" }),

  awful.key({}, "Print", function()
    awful.util.spawn("flameshot_wrapper")
  end, { description = "custom border screenshot", group = "launcher" }),

  awful.key({ modkey }, "-", function()
    awful.util.spawn("wpctl set-volume -l 1.5 @DEFAULT_AUDIO_SINK@ 5%-")
  end, { description = "decrease volume by 5%", group = "launcher" }),
  ...
  end, { description = "swap with next by index", group = "client" }),
...
client.connect_signal("manage", function(c)
  ...
  get_parent_pid(c.pid, function(err, ppid)
    ...
    if
      parent_client
      and (parent_pid:find("(" .. parent_client.pid .. ")"))
      and can_swallow(c.class)
      and cannot_be_swallowed(parent_client.class)
    then

...
class subWindowFilterAll(QMdiSubWindow):
  ...
  def eventFilter(self, obj, e):
    ...
    if e.type() == QEvent.WindowStateChange:
      if Application.readSetting(
        "", "mdi_viewmode", "1"
      ) == "0": #prevent freeze on user changing krita windows mode
        oldMaximized = False
  ...
  def hideAllExcept(self, resizer, exception):
    ...
    if resizer.activeSubwin != None:
      resizer.activeSubwin.setMinimumWidth(0) #minimal width would not allow them to hide properly

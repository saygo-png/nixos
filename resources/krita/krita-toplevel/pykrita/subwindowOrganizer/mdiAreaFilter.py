...
class mdiAreaFilter(QMdiArea):
    ...
    if not sip.isdeleted(self.resizer.mdiArea):
      if e.type() == QEvent.Resize:
        if Application.readSetting("", "mdi_viewmode", "1") == "0":
          ...
          self.sizeBefore = [
            self.resizer.mdiArea.width(),
            self.resizer.mdiArea.height()
  ...
  def viewOpenedEvent(self, resizer):
    ...
    menu = newSubwindow.children()[
      0] #when the addon is enabled, user cant toggle "always on top" action
    ...
    if (resizer.views >= 3 and resizer.refNeeded
        ) or (resizer.views >= 2 and (not resizer.refNeeded)): #open as floating window
  ...
  def moveFloatersOnAreaChange(self, resizer):
    ...
    for subwindow in resizer.mdiArea.subWindowList():
      if subwindow != resizer.activeSubwin and subwindow != resizer.otherSubwin:
        ...
        if x > resizer.mdiArea.width() - subwindow.width(
        ) - x: #closer to the right border of canvas than left one
          x += resizer.mdiArea.width() - self.sizeBefore[0]
        if y > resizer.mdiArea.height() - subwindow.height(
        ) - y: #closer to the right border of canvas than left one

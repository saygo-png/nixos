  ...
  def eventFilter(self, obj, e):
    if e.type() == QEvent.Resize:
      ...
      self.cursor = self.resizer.mdiArea.mapFromGlobal(
        e.globalPos()
      ) #self.cursor in relation to mdiarea
  ...
  def swapFloaters(self, obj):
    for subwindow in self.resizer.mdiArea.subWindowList(): #swapping with floaters
      if self.isUnderMouse(subwindow, self.cursor) and subwindow != obj:
        if subwindow != self.resizer.activeSubwin and subwindow != self.resizer.otherSubwin and not subwindow.isMinimized(
        ): #other floater, swap with it

...
from PyQt5.QtWidgets import QMessageBox, QToolButton, QWidget
...
class resizer:
  ...
  def moveSubwindows(self, checkSizeChange=True):
    ...
    if current != None: #weird situation with maximized background window
      if self.refNeeded and self.activeSubwin != None and self.otherSubwin != None and (
        not current.isMaximized()
      ): #two split windows
  ...
  def userModeOneWindow(self):
    ...
    if self.otherSubwin != None:
      self.otherSubwin.removeEventFilter(
  ...
  def userOpenOverview(self):
    ...
    overview.move(
      self.mdiArea.width() - overview.width(),
